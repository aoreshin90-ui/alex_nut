...
# Supported: Proxmox VE 8.0.x – 8.9.x and all 9.x (9.0, 9.1, 9.2...)
pve_check() {
  local PVE_VER
  ...
  # Check for Proxmox VE 9.x: allow all 9.x
  if [[ "$PVE_VER" =~ ^9\.([0-9]+) ]]; then
    local MINOR="${BASH_REMATCH[1]}"
    if ((MINOR < 0 || MINOR > 99)); then
      msg_error "This version of Proxmox VE is not supported."
      msg_error "Supported: Proxmox VE version 9.x"
      exit 1
    fi
    return 0
  fi
